...
export default class ImageFilterPage extends React.Component {
    ...
    _renderMainImageIndicator = () => {
        const { mainImageInit } = this.state;
        if (!mainImageInit) {
            return (
                <BallIndicator size={40} />
            );
        }
        return null;
    }
    ...
    _filterInitialized = filterName => {
        ...
        this.setState({
            filterInitProcess: filterInitProcess
        });
    }

    _checkFilterInitStatus = () => {
        const { filterInitProcess } = this.state;
        if (filterInitProcess.length < 6) {
            return false;
        }
        return true
    }
    ...
    renderFilterImage = (filterName, key) => {
        ...
        return (
            <TouchableOpacity
                ...
                <View style={[
                    styles.filterImage,
                    { borderColor: filterSelection === filterName ? theme.primaryColor : null },
    ...
    renderMetaModal = () => {
        return (
            <Modal
                visible={this.state.metaModalVisible}
                style={modalStyle}
            >
                <View style={styles.modalContent}>
                    <MyHeader
